...
#system(cmd)
#cmd <- sprintf("cd %s/bin; cat ../theta_pmcmc_det3.json | ./pmcmc -J 1 -M 100000  --trace --traj --hat -I 1 --root ../%s > ../%s/theta_pmcmc_det4.json",dir_model,dir_det, dir_model)
#system(cmd)
...
#system(cmd)
#cmd <- sprintf("cd %s/bin; cat ../theta_pmcmc_det5.json | ./pmcmc -J 1 -M 100000  --switch 500000 --trace --traj --hat -I 1 --root ../%s > ../%s/theta_pmcmc_det6.json",dir_model,dir_det, dir_model)
#system(cmd)

# SIMULATE FROM POSTERIOR
cmd <- sprintf(" cd %s/bin; ssm-predict ../theta_pmcmc_det6.json ../mcmc/X_1.csv ../mcmc/trace_1.csv 2002-01-07 | ./simul -I 128 --start 2002-01-07  --end 2016-01-01 --verbose --freq 7 --traj --root ../%s",dir_model,dir_det)
system(cmd)
