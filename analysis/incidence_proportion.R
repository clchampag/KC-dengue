...
ar=ggplot(mdata,aes_string(x='date')) +geom_point(aes_string( y="value", group="variable", color="variable"),size=4)+
  scale_colour_manual(values=c("skyblue", "forestgreen", "darkblue","dodgerblue","red","orange"),
                      name="",
  ...
  geom_errorbar(data=AR_laneri,aes_string(ymin= "q025_lan", ymax="q975_lan"),color="forestgreen",alpha=1, size=1.5) +
  geom_errorbar(data=AR_pandey,aes_string(ymin= "q025_pandey", ymax="q975_pandey"),color="darkblue",alpha=1, size=1.5) +
  geom_errorbar(data=AR_seiar,aes_string(ymin= "q025_seiar", ymax="q975_seiar"),color="dodgerblue",alpha=1, size=1.5) +
